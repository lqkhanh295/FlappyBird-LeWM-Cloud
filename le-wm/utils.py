...
def get_img_preprocessor(source: str, target: str, img_size: int = 224):
    ...
    resize = dt.transforms.WrapTorchTransform(torchvision.transforms.Resize(img_size, antialias=True), source=source, target=target)
...
class SaveCkptCallback(Callback):
    ...
    def _save(self, model, epoch):
        ...
        save_dir = Path(self.run_name)
        save_dir.mkdir(parents=True, exist_ok=True)
        save_path = save_dir / f'weights_epoch_{epoch}.pt'
